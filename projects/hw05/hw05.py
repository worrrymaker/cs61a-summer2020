...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,password2):
        if len(err_ls) == 3:
            return f"Too many incorrect attempts. Attempts: {err_ls}"
        nonlocal balance
        if password2 != password:
            if len(err_ls)<3:
                err_ls.append(password2)
...
def merge(incr_a, incr_b):
    ...
    a,b = next(iter_a,float('inf')),next(iter_b,float('inf'))
    while (a!=float('inf')) or (b!=float('inf')):
        if a == b:
            yield a
            a,b = next(iter_a,float('inf')),next(iter_b,float('inf'))
        elif a > b:
            yield b
            b = next(iter_b,float('inf'))
        else :
            yield a
            a = next(iter_a,float('inf'))
