...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.balance >= self.price:
            back = self.balance-self.price
            self.stock -=1
            self.balance = 0
            if back != 0:
                return 'Here is your {0} and ${1} change.'.format(self.pdt,back)
            return 'Here is your {0}.'.format(self.pdt)
        return 'You must add ${0} more funds.'.format(self.price-self.balance)

    def restock(self,num):
        self.stock+=num
        return  'Current {0} stock: {1}'.format(self.pdt,self.stock)
...
def path_yielder(t, value):
    ...
    if t.label == value:
        yield [t.label]
    for _ in t.branches:
        for path in path_yielder(_,value):
            yield [t.label]+path
