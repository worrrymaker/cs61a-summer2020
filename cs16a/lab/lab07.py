...
def trade(first, second):
    ...
    equal_prefix = lambda: sum([_ for _ in first[:m]]) == sum([_ for _ in second[:n]])
    while not equal_prefix() and m<=len(first) and n <=len(second):
        if sum([_ for _ in first[:m]]) < sum([_ for _ in second[:n]]):
            m += 1
        else:
            n += 1
...
def make_glookup(class_assignments):
    ...
    """
    "*** YOUR CODE HERE ***"
    tem_point ,total_point = 0,0
    def func(key,point):
        nonlocal tem_point,total_point
        total_point+=class_assignments[key]
        tem_point +=point
        return tem_point/total_point
    return func
